(function () {
  ...
  function controllerFunction($scope, $http, $timeout) {
    ...
    function getAll() {
      $http.get("http://localhost:8080/Spring4MVCAngularJSExample/api/manga")
...
/*    vm.songManagerView = true;
    ...
    vm.listSelected = {};
    vm.listSelected.list = new Set([]);
    vm.listObject = musicManagerService.findAllSongs();
    vm.listCol = musicManagerService.getSongCols();
    vm.ctrlButton = {};
    ...
    function viewAll() {
      vm.songManagerView = true;
        updateCtrlButton(vm.listSelected.list.size);
    }
    function addSong() {
      vm.songManagerView = false;
      vm.action = 'Create';
      vm.listSelected.song = null;
      vm.listSelected.list.clear();
    ...
    function deleteSong() {
      ...
      var listId = Array.from(vm.listSelected.list);
    ...
    function disableAllCtrButton(){
        vm.ctrlButton.add = true;
        vm.ctrlButton.edit = true;
        vm.ctrlButton.delete = true;
    }
    $scope.$watch(
      'vm.listSelected.list.size',
      function (newVal) {
        updateCtrlButton(newVal);
      });
      function updateCtrlButton(sizeSelected){
          if(sizeSelected < 1) {
              vm.ctrlButton.add = false;
              vm.ctrlButton.edit = true;
              vm.ctrlButton.delete = true;
          } else if(sizeSelected == 1) {
              vm.ctrlButton.add = false;
              vm.ctrlButton.edit = false;
